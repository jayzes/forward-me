...
describe 'forwarding inbound e-mails', job: true do

  before do
    ENV['FORWARDING_FROM_ADDRESS'] = 'forwarder@example.com'
  end
  ...
  describe 'when a relevant forwarding address is found in the DB' do
    let!(:forwarding_address) { ForwardingAddress.create! source_emails: [ 'info@example.com' ], recipient_emails: [ 'jay@example.com', 'hello@me.com' ] }

    before do
      post '/email_processor', params
    end
    ...
    it 'sends a forwarded e-mail' do
      ...
      expect(email.reply_to).to include('jay.zeschin@example.com')
      expect(email.body.parts.first.body.decoded).to eq('This is a test...')
      expect(email.body.parts.last.body.decoded).to eq(params['html'])
    end
  end

  describe 'when no relevant forwarding address is found in the DB' do
